...
void ota_task(void *pvParameter){
    ...
    while (1) {
        ...
        if (total_size > 0) {
            ...
            if (percent != last_percent) {
                ...
                int send_data_len = sprintf(send_data , "%s%d\n" , updateFirmwareSteps[UPDATE_FW_UPDATING].response, percent);
    ...
    if (esp_https_ota_is_complete_data_received(https_ota_handle) == true) {
        ...
        int send_data_len = sprintf(send_data , "%s\n", updateFirmwareSteps[UPDATE_FW_UPDATED].response);
        send(sock, send_data, send_data_len, 0);
